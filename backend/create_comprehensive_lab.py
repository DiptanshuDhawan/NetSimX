...
print("Using Router Template:", router_temp["name"] if router_temp else "None")
print("Using Switch Template:", switch_temp["name"] if switch_temp else "None")

# Create Nodes
print("Creating Nodes...")
r1_res = requests.post(f"{URL}/projects/{project_id}/templates/{router_temp['template_id']}", json={"x": -200, "y": -150, "name": "R1", "compute_id": "local"}, auth=AUTH)
r1 = r1_res.json()

r2_res = requests.post(f"{URL}/projects/{project_id}/templates/{router_temp['template_id']}", json={"x": 200, "y": -150, "name": "R2", "compute_id": "local"}, auth=AUTH)
r2 = r2_res.json()

s1_res = requests.post(f"{URL}/projects/{project_id}/templates/{switch_temp['template_id']}", json={"x": -200, "y": 100, "name": "S1", "compute_id": "local"}, auth=AUTH)
s1 = s1_res.json()

pc1_res = requests.post(f"{URL}/projects/{project_id}/templates/{vpcs_temp['template_id']}", json={"x": -350, "y": 250, "name": "PC1", "compute_id": "local"}, auth=AUTH)
pc1 = pc1_res.json()

pc2_res = requests.post(f"{URL}/projects/{project_id}/templates/{vpcs_temp['template_id']}", json={"x": -50, "y": 250, "name": "PC2", "compute_id": "local"}, auth=AUTH)
pc2 = pc2_res.json()
...
# R1 e0/1 <-> R2 e0/1 (OSPF Link)
create_link(r1, 1, r2, 1)

# R1 e0/0 <-> S1 e0/0 (Trunk Link for Router-on-a-stick)
create_link(r1, 0, s1, 0)

# S1 e0/1 <-> PC1 e0/0 (VLAN 10 Access)
create_link(s1, 1, pc1, 0)

# S1 e0/2 <-> PC2 e0/0 (VLAN 20 Access)
create_link(s1, 2, pc2, 0)
